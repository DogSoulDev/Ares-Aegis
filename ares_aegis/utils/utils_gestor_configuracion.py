...
import logging
from .utils_ayuda_logging import configurar_logger_modulo
from .utils_ayuda_rutas import crear_ruta_segura
...
class ConfiguracionSIEM:
    ...
    tipos_eventos_criticos: Optional[List[str]] = None
    ...
    def __post_init__(self):
        if self.tipos_eventos_criticos is None:
            self.tipos_eventos_criticos = ["AMENAZA_DETECTADA", "MALWARE_DETECTADO", "ACCESO_NO_AUTORIZADO"]

@dataclass
class ConfiguracionEscaneador:
    extensiones_peligrosas: Optional[List[str]] = None
    ...
    def __post_init__(self):
        if self.extensiones_peligrosas is None:
            self.extensiones_peligrosas = [".exe", ".bat", ".cmd", ".scr", ".pif", ".com", ".js", ".vbs", ".ps1", ".jar", ".dll"]


@dataclass
class ConfiguracionFIM:
    rutas_monitoreadas: Optional[List[str]] = None
    rutas_criticas_sistema: Optional[List[str]] = None
    rutas_excluidas: Optional[List[str]] = None
...
class ConfiguracionMonitorRed:
    ...
    puertos_criticos: Optional[List[int]] = None
    puertos_pentesting: Optional[List[int]] = None
    ips_whitelist: Optional[List[str]] = None
...
class ConfiguracionGlobal:
    ...
    siem: Optional[ConfiguracionSIEM] = None
    escaneador: Optional[ConfiguracionEscaneador] = None
    fim: Optional[ConfiguracionFIM] = None
    monitor_red: Optional[ConfiguracionMonitorRed] = None
    cuarentena: Optional[ConfiguracionCuarentena] = None
    reportes: Optional[ConfiguracionReportes] = None
...
class GestorConfiguracion:
    ...
    def validar_configuracion(self) -> List[str]:
        ...
        try:
            ...
            if config.siem and config.siem.ventana_tiempo <= 0:
                errores.append("SIEM: ventana_tiempo debe ser mayor a 0")
            
            if config.escaneador and config.escaneador.tamaño_maximo_archivo_mb <= 0:
                errores.append("Escaneador: tamaño_maximo_archivo_mb debe ser mayor a 0")
            
            if config.fim and config.fim.intervalo_verificacion_minutos <= 0:
                errores.append("FIM: intervalo_verificacion_minutos debe ser mayor a 0")
            
            # Validar directorios
            if config.fim and config.fim.rutas_monitoreadas:
